...
// Mémorisé pour éviter les re-renders inutiles lors du filtrage
const ArtworkThumbnail = memo(
    ...
    return (
      ...
      >
        <Link
          ...
          className="relative w-full h-full rounded-lg overflow-hidden bg-gray-200 dark:bg-gray-700 cursor-pointer transition-transform hover:scale-105 block"
  ...
  (prevProps, nextProps) => {
    ...
    return (
      prevProps.artworkId === nextProps.artworkId &&
...
// Composant pour une collection individuelle avec son propre état de filtre
function CollectionCard({
  collection,
}: {
  collection: PhysicalCollectionWithItems;
}) {
  const [showOnlyAvailable, setShowOnlyAvailable] = useState(false);

  // Filtrer les œuvres selon le commercialStatus si le filtre est activé
  // Utiliser useMemo pour éviter les recalculs inutiles et maintenir la stabilité
  const filteredItems = useMemo(
    () =>
      showOnlyAvailable
        ? collection.physicalItems.filter(
            (item) => item.commercialStatus === "AVAILABLE"
          )
        : collection.physicalItems,
    [collection.physicalItems, showOnlyAvailable]
  );

  const onlineItems = useMemo(
    () => filteredItems.filter((item) => item.isOnline),
    [filteredItems]
  );
    ...
    <div className="bg-gradient-to-br from-white to-gray-50 dark:from-gray-800 dark:to-gray-900 rounded-2xl overflow-hidden shadow-[0_1px_3px_rgba(0,0,0,0.05),0_4px_12px_rgba(0,0,0,0.04),0_0_0_1px_rgba(0,0,0,0.02)] border border-gray-200/80 dark:border-gray-700/50 transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_4px_6px_rgba(0,0,0,0.07),0_10px_24px_rgba(0,0,0,0.08),0_0_0_1px_rgba(59,130,246,0.1)] hover:border-blue-200 dark:hover:border-blue-400/40 relative group">
      ...
      <div className="p-6 px-7">
        ...
        <div className="mb-6 flex items-center gap-2">
          <input
            type="checkbox"
            id={`filter-available-${collection.id}`}
          ...
          />
          <label
            htmlFor={`filter-available-${collection.id}`}
            className="text-sm font-medium text-gray-700 dark:text-gray-300 cursor-pointer select-none"
          >
            Afficher uniquement les œuvres disponibles à la vente
          </label>
        </div>
        ...
        {onlineItems.length > 0 && (
          <div className="mb-8 last:mb-0">
            <h3 className="text-base font-semibold mb-4 text-gray-900 dark:text-gray-100">
              Œuvres présentées sur SINGULART actuellement ({soldItems.length}{" "}
              vendues)
            </h3>
            <div className="flex flex-wrap gap-4 md:gap-3">
              {onlineItems.map((physicalItem) => (
                <ArtworkThumbnail
                  key={String(physicalItem.item.id)}
        ...
        {filteredItems.length === 0 && (
          <div className="text-center py-12 px-4 text-gray-500 dark:text-gray-400">
            <p>
              {showOnlyAvailable
                ? "Aucune œuvre disponible à la vente dans cette collection"
