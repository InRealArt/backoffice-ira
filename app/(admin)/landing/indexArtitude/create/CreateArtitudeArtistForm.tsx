...
// Autocomplétion d'adresse Google Places (hooks/useGooglePlaces.ts) désactivée :
// la facturation n'est pas activée sur le projet Google Cloud associé à la clé
// NEXT_PUBLIC_GOOGLE_MAPS_API_KEY (BillingNotEnabledMapError). Le hook reste
// disponible et prêt à être rebranché si la facturation est activée un jour.
...
type ArtistOption = ArtistName & { id: number }

interface CreateArtitudeArtistFormProps {
...
export default function CreateArtitudeArtistForm({
  artists,
  countries,
}: CreateArtitudeArtistFormProps) {
  ...
  const WEEKDAYS: OpeningHoursPeriod['day'][] = [
    'MONDAY',
    'TUESDAY',
    'WEDNESDAY',
    'THURSDAY',
  ...
  const [hours, setHours] = useState<Record<OpeningHoursPeriod['day'], DayHours>>(() =>
    DAYS.reduce((acc, day) => {
      acc[day.key] = {
        enabled: WEEKDAYS.includes(day.key),
        openTime: '10:00',
        closeTime: '17:00',
      }
      return acc
  ...
  >([
    ...
    { id: 'creation', label: 'Création de la fiche Artitude', status: 'pending' },
  ...
  const [geocodingStatus, setGeocodingStatus] = useState<'idle' | 'loading' | 'found' | 'not-found'>('idle')
  ...
  const uploadCoverImage = async (file: File, folderName: string): Promise<string> => {
    const { uploadArtitudeArtistImage } = await import('@/lib/r2/storage')
  ...
  const onSubmit = async (data: FormValues) => {
    ...
    // Accumule tous les chemins déjà uploadés (durablement écrits sur R2) pour
    // pouvoir les nettoyer si une catégorie suivante ou la création DB échoue,
    ...
    try {
      ...
      try {
        if (coverFile) {
          coverImage = await uploadCoverImage(coverFile, folderName)
          uploadedPaths.push(coverImage)
        }
        interiorImages = await uploadCategory(interiorFiles, 'interior', folderName)
        uploadedPaths.push(...interiorImages)
        exteriorImages = await uploadCategory(exteriorFiles, 'exterior', folderName)
        uploadedPaths.push(...exteriorImages)
        artistImages = await uploadCategory(artistFiles, 'artist', folderName)
        uploadedPaths.push(...artistImages)
        otherImages = await uploadCategory(otherFiles, 'others', folderName)
        uploadedPaths.push(...otherImages)
      ...
      const result = await createArtitudeArtistAction({
      ...
      if (result.success) {
        ...
        success('Fiche Artitude créée avec succès')
      ...
      console.error('Erreur lors de la création de la fiche Artitude:', err)
      const errorMessage =
        err?.message || 'Une erreur est survenue lors de la création de la fiche Artitude'
      updateStepStatus('creation', 'error')
  ...
  const photosTabContent = selectedArtist && (
    <div className="form-section">
      <p className="section-subtitle">
        Les images seront converties en WebP et stockées dans /ARTITUDE/{getArtistFullName(selectedArtist)}/
      </p>
    ...
    <div className="page-container">
      <div className="page-header">
        <div className="header-top-section">
          <h1 className="page-title">Ajouter une fiche Artitude</h1>
        </div>
        <p className="page-subtitle">
          Sélectionnez un artiste et renseignez sa fiche établissement Artitude
        </p>
      </div>
        ...
        <div className="form-card">
          <div className="card-content">
            ...
            <div className="form-group">
              ...
              {errors.artistId && <p className="form-error">{errors.artistId.message}</p>}
            </div>
