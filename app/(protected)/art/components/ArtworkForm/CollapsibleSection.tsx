...
export default function CollapsibleSection({ 
  ...
  return (
    <div
      className={`w-full mb-6 rounded-r-xl border border-gray-200 ${bgClass} shadow-sm transition-all duration-300 hover:shadow-md dark:border-gray-700 ${className}`}
      style={{ overflow: 'visible' }}
    >
      <button
        type="button"
        onClick={() => setIsExpanded(!isExpanded)}
        className={`w-full flex items-center justify-between p-6 text-left focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2 rounded-tr-xl transition-colors duration-200 ${hoverClass}`}
      >
        <h3 className="text-lg font-semibold text-gray-900 dark:text-gray-100 flex items-center gap-3 min-w-0">
          <span className="w-1 h-6 bg-primary rounded-full flex-shrink-0"></span>
      ...
      >
        <div className="w-full px-6 pb-6 pt-4 border-t border-gray-100 dark:border-gray-700" style={{ overflow: 'visible' }}>
          {children}
        </div>
      </div>
    </div>
